...
def build(config):
    G = nx.DiGraph(
        ...
        loss_tensors={
            ...
            'D': LSSD+OUTP,},
        train_sets={
            'G': [GNF0, GNF1, GNF2, GNF3, GNF4, BRGF],
            'R': [BRGR, GNR4, GNR3, GNR2, GNR1, GNR0],
            'D': [BRGD, DSC4, DSC3, DSC2, DSC1, DSC0, DGN0, DGN1, DGN2, DGN3, DGN4, BRGU],},
        img_pairs=[
            ('G', BRGF+OUTP),
            ('R', BRGF+OUT2),
            ('A_G', SPLT+GOUT),
    ...
    G.add_edges_from([
        # Reverse path
        (BRGR, GNR4, {'out': OUTP, 'in': INPT}),
        (GNR4, GNR3, {'out': OUTP, 'in': INPT}),
        ...
        (GNF3, GNF4, {'out': [OUTP, OUT2], 'in': [INPT, INP2]}),
        (GNF4, BRGF, {'out': [OUTP, OUT2], 'in': [INPT, INP2]}),
        (BRGF, LSSR, {'out': [OUTP, OUT2], 'in': [O_IN, A_IN]}),
        # Main path
        (BRGF, CNCT, {'out': OUTP, 'in': G_IN}),
        (CNCT, BRGD, {'out': OUTP, 'in': INPT}),
        (BRGD, DSC4, {'out': OUTP, 'in': INPT}),
        (DSC4, DSC3, {'out': OUTP, 'in': INPT}),
        ...
        (DGN3, DGN4, {'out': OUTP, 'in': INPT}),
        (DGN4, BRGU, {'out': OUTP, 'in': INPT}),
