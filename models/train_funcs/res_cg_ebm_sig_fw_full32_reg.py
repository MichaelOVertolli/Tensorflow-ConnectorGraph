...
def build_train_ops(log_dir, conngraph, inputs, outputs,
    ...
    with tf.Session(graph=tf.Graph()) as sess:
        ...
        with tf.variable_scope(train_scope) as vs:
            ...
            lm_n = tf.Variable(config.lambda_n, trainable=False, name='lambda_noise')
            ...
            n_losses = sess.graph.get_tensor_by_name(loss_tensors['N'])
            n_l2 = tf.identity(n_losses[0], name='n_l2')
            n_gms = tf.identity(n_losses[1], name='n_gms')
            n_chrom = tf.identity(n_losses[2], name='n_chrom')
            
            n_loss = n_l2 + n_gms*lm_gms + n_chrom*lm_chrom
            n_loss = tf.identity(n_loss, name='n_loss')
            ...
            d_out = d_loss - k_t * g_loss + n_loss*lm_n
            ...
            summary_set.extend([
                ...
                tf.summary.scalar('loss/n_l2', n_l2),
                tf.summary.scalar('loss/n_gms', n_gms),
                tf.summary.scalar('loss/n_chrom', n_chrom),
                tf.summary.scalar('loss/n_loss', n_loss),
        ...
        tf.add_to_collection('outputs_interim', n_loss)
...
def build_feed_func(gen_tensor, gen_input, rev_input, data_inputs, noise_input, alpha_tensor, **keys):
    def get_feed_dict(self, trainer):
        ...
        for i in range(trainer.batch_size):
            for j in range(channels):
                x_noise[i, j, :, :] = gaussian_filter(x[i, j, :, :], sigma=1) # sigma=1 is set by denoise BEGAN paper

        feeds.append((noise_input, x_noise))
...
def build_send_func(gen_input, rev_input, data_inputs, noise_input, gen_outputs, a_output, **keys):
    def send_outputs(self, trainer, step):
        ...
        feeds = [(gen_input, z_fixed), (noise_input, x_fixed)]
        feeds = dict(feeds)
        ...
        #autoencode
        for k, img in (('real', x_fixed), ('gen', gen)):
            if img is None:
                continue
        ...
        for _, ratio in enumerate(np.linspace(0, 1, 10)):
            ...
            #generate
            feeds = [(gen_input, z), (noise_input, x_fixed)]
            feeds = dict(feeds)
