...
def build_train_ops(log_dir, conngraph, inputs, outputs,
    ...
    with tf.Session(graph=tf.Graph()) as sess:
        ...
        # weights = sess.graph.get_tensor_by_name('res_gen_pair_08/G/Conv_1/weights:0')
        # w = sess.run(weights)
        # print w[0,0,:,:]
        # tester = tf.assign(weights, np.ones([3, 3, 128, 128]), name='tester1234')
...
def build_send_func(gen_input, rev_input, data_inputs, gen_outputs, a_output, **keys):
    def send_outputs(self, trainer, step):
        ...
        for name, output in gen_outputs.items():
            ...
            # if np.mean(np.absolute(x_gen[0, :, :, :] - x_gen[15, :, :, :])) < 0.005:
