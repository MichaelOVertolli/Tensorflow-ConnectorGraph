...
def config(type_):
    ...
    # m = re.search('(?<={})\de-\d'.format(GLRN), type_)
    # if m is not None:
    #     config.g_lr = float(m.group(0))
    # else:
    #     raise ConfigError('Invalid g_lr in type: {}.'.format(type_))
    # m = re.search('(?<={})\de-\d'.format(DLRN), type_)
    # if m is not None:
    #     config.d_lr = float(m.group(0))
    # else:
    #     raise ConfigError('Invalid d_lr in type: {}.'.format(type_))
    ...
    config.alpha_update_steps = 15000
