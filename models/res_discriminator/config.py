...
def config(type_):
    ...
    if PROJ in type_:
        ...
        config.hidden_nums = [min(2**(max_-i), config.hidden_num) for i in range(config.repeat_num)]
    else:
        config.project = False
        config.hidden_nums = [config.hidden_num for i in range(config.repeat_num)]
    if ALPH in type_:
        config.alpha = True
    else:
        config.alpha = False
    ...
    config.sizes = [2**i for i in range(2, int(np.log2(config.size)+1))]
    config.hidden_nums.reverse()
    config.sizes.reverse()
    config.name = type_
