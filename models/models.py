...
def GeneratorSkipCNN(z, hidden_num, output_num, repeat_num, alphas, data_format, reuse):
    with tf.variable_scope("G", reuse=reuse) as vs:
        ...
        out_set = []
        for idx in range(repeat_num):
            x = slim.conv2d(x, hidden_num, 3, 1, activation_fn=tf.nn.elu, weights_initializer=tf.random_normal_initializer(),
                            normalizer_fn=slim.unit_norm, normalizer_params={'dim':1, 'epsilon':1e-8}, data_format=data_format)
            x = slim.conv2d(x, hidden_num, 3, 1, activation_fn=tf.nn.elu, weights_initializer=tf.random_normal_initializer(),
                            normalizer_fn=slim.unit_norm, normalizer_params={'dim':1, 'epsilon':1e-8}, data_format=data_format)
            if idx < repeat_num - 1:
...
def DiscriminatorSkipCNN(xs, input_channel, z_num, repeat_num, hidden_num, data_format):
    with tf.variable_scope("D") as vs:
        ...
        for i in range(1, len(xs_)):
            xs_[i] = slim.conv2d(xs_[i], hidden_num*i, 3, 1, activation_fn=tf.nn.elu, weights_initializer=tf.random_normal_initializer(),
                                 data_format=data_format)
        x = slim.conv2d(xs_[0], hidden_num, 3, 1, activation_fn=tf.nn.elu, weights_initializer=tf.random_normal_initializer(),
                        data_format=data_format)
        prev_channel_num = hidden_num
        for idx in range(repeat_num):
            channel_num = hidden_num * (idx + 1)
            x = slim.conv2d(x, channel_num, 3, 1, activation_fn=tf.nn.elu, weights_initializer=tf.random_normal_initializer(),
                            data_format=data_format)
            x = slim.conv2d(x, channel_num, 3, 1, activation_fn=tf.nn.elu, weights_initializer=tf.random_normal_initializer(),
                            data_format=data_format)
            if idx < repeat_num - 1:
                x = slim.conv2d(x, channel_num, 3, 2, activation_fn=tf.nn.elu, weights_initializer=tf.random_normal_initializer(),
                                data_format=data_format)
                x = x + xs_[idx+1]
                #x = tf.contrib.layers.max_pool2d(x, [2, 2], [2, 2], padding='VALID')
        ...
        out_set = []
        for idx in range(repeat_num):
            x = slim.conv2d(x, hidden_num, 3, 1, activation_fn=tf.nn.elu, weights_initializer=tf.random_normal_initializer(), data_format=data_format)
            x = slim.conv2d(x, hidden_num, 3, 1, activation_fn=tf.nn.elu, weights_initializer=tf.random_normal_initializer(), data_format=data_format)
            out_set.append(x)
            if idx < repeat_num - 1:
                x = upscale(x, 2, data_format)
