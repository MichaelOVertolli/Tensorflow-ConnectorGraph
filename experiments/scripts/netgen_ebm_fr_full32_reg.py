...
def test():
    ...
    dlr = 6e-6
    rlr = 6e-5
    n = NetGen('res_cg_ebm_fr_full32_reg',
               'b16_z128_sz32_h128_gms0.0_chrom0.0_g0.{}_elu_pconv_wxav'.format(gamma),
               'celebc/fr_full32_reg',
               [{'dir': 'base_block', 'data': 'celeb_trainr', 'glr': glr, 'dlr': dlr, 'rlr': rlr, 'fetch_size': 5000, 'resize': [32, 32]},],
               branching=False)
    n.run()
    event_dir = sorted(glob.glob(os.path.join(n.log_dir, 'base_block', '*')))[0]
    write_data('./logs/celebc/fr_full32_reg', event_dir,
