...
def test():
    ...
    log_dir_ = '/home/olias/data/mnist_avgdists'
    with open(os.path.join(log_dir_, 'alldists.pkl'), 'r') as f:
        alldists = pickle.load(f)
    centers = []
    for k in alldists:
        c, _ = alldists[k]
        centers.append(np.expand_dims(c, axis=2))
    centers = np.stack(centers)
    centers = norm_img(np.transpose(centers, [0, 3, 1, 2]))
    try:
    ...
    feed_dict[G.graph['rev_input']] = centers
    latents, images = nr.sess.run([rev_output, G.graph['gen_outputs']['R']], feed_dict)
    images = denorm_img_numpy(images, 'NCHW')
    print [k for k in alldists]
    print centers.shape, images.shape
    save_image(images, './logs/mnist_netgen_centers.jpg')
    np.save(os.path.join(model_dir, 'centers.npy'), images)
    np.save(os.path.join(model_dir, 'clatents.npy'), latents)
